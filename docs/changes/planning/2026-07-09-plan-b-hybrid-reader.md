...
1. **W1 契約**：依 blueprint §2 寫 `lib/features/reader_v2/hybrid/core/hybrid_types.dart` + `hybrid_contracts.dart`（一個 sonnet 代理 + 人工審查）。
2. **W2 模組並行**：五個代理（A measure / B text / C paragraph+pump / D view / E anchor+overlay+progress+telemetry）依所有權表並行實作 + 各自單元測試；只依賴 core 契約，不互相 import。
3. **W3 整合**：單一代理寫 `hybrid_reader_screen.dart` 落實 D5 全部條款，改 `reader_v2_page.dart` 切換點與 D6 進度顯示。
4. **W4 驗證迴圈**：`flutter analyze` + `flutter test` 修到全綠。
5. **W5 審查**：I1–I6 不變量審查 + 正確性審查，修復。
6. **W6 清理**：依 D8 刪舊碼與綁死測試、補行為契約測試、更新 atlas（reader.md + index Architecture Decisions），plan 移 completed，commit+push。
