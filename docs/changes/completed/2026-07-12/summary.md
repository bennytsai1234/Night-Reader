...
- [reader-bottom-menu-scrub-redesign](reader-bottom-menu-scrub-redesign.md) — 閱讀器底部選單重整：主列 5→4 顆（換源移入進階設定）、自動翻頁速度移入進階設定並把下限 8%→2%、拖動條改為章內進度（0–100%，放開才跳）；analyze/test 全過，待真機回歸。
