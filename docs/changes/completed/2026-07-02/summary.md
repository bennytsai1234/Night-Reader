...
- [reader-fling-settle-jank](reader-fling-settle-jank.md) — 修復滾動放開後 fling 減速不流暢：`Positioned` 補 tile key 讓頁面位移不再整視埠重繪、`shouldRepaint` 只比對繪製內容（背景排版推進不再重繪未變 tile）、動作中 capture 的 notify 加 200ms 節流；新增 3 個重繪/節流回歸測試（紅燈驗證通過），`flutter test` 658 全綠。
