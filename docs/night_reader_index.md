...
- **reader**：擁有 Reader V2 閱讀器主流程，八層架構（`features/reader_v2`：shell / application / runtime / content / layout / render / viewport / features 子面板含 TTS、settings、menu、auto_page、bookmark、replace_rule）。release 重點回歸區。未來工作從這裡開始：排版/渲染、章節預載與進度、TTS 逐詞高亮、閱讀設定、點擊區、書籤、章內替換、換源 sheet。症狀指向：排版錯位/崩潰、翻頁卡頓、預載失敗、TTS 高亮偏移、設定不保留。
