...
初始化時為空。跨模組決策記錄於此；模組級別決策記錄於各模組文件的 Known Risks 或 Do Not Do 欄位。
...
所有書源規則的解析與執行底層：AnalyzeRule、AnalyzeUrl、CSS/XPath/JSONPath/Regex 解析器、JS 引擎、web_book 服務。修改規則解析邏輯、新增 JS 擴充、排查書源解析失敗，從這裡開始。

**書源管理** (`lib/features/source_manager/` + `lib/core/services/book_source_service.dart`, `source_*.dart`)
書源的完整生命週期：新增、刪除、匯入、匯出、有效性驗證、訂閱更新、偵錯。修改書源管理頁、書源驗證流程、訂閱更新行為，從這裡開始。

**閱讀器 V2** (`lib/features/reader_v2/`)
閱讀頁面的全部功能：排版引擎、Canvas 渲染、viewport（捲動/滑動）、runtime 狀態機、TTS 朗讀、設定面板、書籤、替換規則。最複雜的模組，修改閱讀體驗從這裡開始。

**書架與書籍** (`lib/features/bookshelf/`, `book_detail/`, `bookmark/`, `read_record/`, `replace_rule/` + 相關 core services/models/DAO)
書架顯示、書籍詳情、閱讀紀錄、書籤管理、全域替換規則。修改書架 UI、書籍資料結構、閱讀進度，從這裡開始。
...
Drift 資料庫、HTTP client、DI 容器、儲存路徑、應用啟動、導覽、主題、版本更新、深連結。所有模組的底層依賴。修改 DB schema、新增 DAO、修改啟動流程，從這裡開始。
