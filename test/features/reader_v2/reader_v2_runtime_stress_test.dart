...
void main() {
  ...
  group('ReaderV2Runtime 壓力測試', () {
    ...
    test(
      'a pending bookmark jump survives presentation before content completes',
      () async {
    ...
    test(
      'disposing a runtime expires its awaiting viewport operation',
      () async {
        final harness = makeRuntime([chapter(0)]);
        final runtime = harness.runtime;
        final owner = Object();
        final entered = Completer<void>();
        final restore = Completer<bool>();
        runtime.registerHybridViewport(owner);
        runtime.registerViewportRestore(owner, (_) {
          entered.complete();
