...
void main() {
  ...
  testWidgets('block 卸載後段落補建不得回呼已 detach 的 render object', (tester) async {
    final store = MeasurementStore();
    final cache = ParagraphCache();
    addTearDown(cache.dispose);
    ...
    // detach 必須釋放 consumer lease：put 若打到已卸載的 render object 會炸 assert。
    cache.put(key, epoch, _paragraph('晚到的段落'));
    expect(tester.takeException(), isNull);
  });
  testWidgets(
    'mounted render owns its paragraph across LRU eviction and replacement',
    (tester) async {
      final store = MeasurementStore();
      final cache = ParagraphCache(capacity: 1);
      addTearDown(cache.dispose);
      final first = _paragraph('first');
      cache.put(key, epoch, first);
      await pumpBlock(tester, store, cache);
      final render = tester.renderObject<RenderCachedBlock>(
        find.byType(CachedBlockWidget),
      );
      cache.put(
      ...
      expect(render, paints..paragraph());
      final replacement = _paragraph('replacement');
      cache.put(key, epoch, replacement);
      expect(first.debugDisposed, true);
      expect(render.debugNeedsPaint, true);
      await tester.pump();
      expect(render, paints..paragraph());
      cache.dispose();
      expect(replacement.debugDisposed, false);
      await tester.pumpWidget(const SizedBox.shrink());
