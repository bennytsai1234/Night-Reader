...
''');
  ...
  test('getChapterListAwait preserves distinct source URLs despite equal adjacent titles', () async {
    requestHandler = (request) async {
...
''');
    ...
    expect(chapters.map((chapter) => chapter.index).toList(), <int>[0, 1, 2, 3, 4, 5]);
    expect(chapters.map((chapter) => chapter.url).toSet(), hasLength(6));
  });
